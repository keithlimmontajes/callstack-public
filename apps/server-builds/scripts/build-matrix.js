...
function createAppJSON(matrix) {
  ...
  for (const version in matrix.versions) {
    ...
    for (const appName in matrix.versions[version]) {
      ...
      const iosBundleURL = `${appInfo.url}/mini-app/${appName}?platform=ios&version=${appVersion}&file=[name][ext]`;
      const androidBundleURL = `${appInfo.url}/mini-app/${appName}?platform=android&version=${appVersion}&file=[name][ext]`;
